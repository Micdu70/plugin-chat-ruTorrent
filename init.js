...
plugin.parseText = function(text)
{
    ...
    for (var i = 0; i < matches.length; i++) {
        ...
        if (matches[i].match(/https?:\/\/|ftps?:\/\/|ftpes:\/\//))
            var url = matches[i];
        else if (matches[i].match(/[-0-9a-zA-Z.+_]+@/))
            var url = "mailto:" + matches[i];
        else
        ...
        theDialogManager.make("chatSmileys", theUILang.chatSmileys,
            "<table class='smileysContainer'>"+
                "<tr>"+
                    ...
                    "<td><span id='angry" + plugin.settings["smileySet"] + "' title='xD'></span></td>"+
